...
import infer_tool_copy_test
from infer_tool_copy_test import Svc
...
def main():
    ...
    parser.add_argument('-s','--target_singer_wav', type=str, default=['singer1'], help='The target singer')
    ...
    keys = args.keys
    target_singer_wav = args.target_singer_wav
    slice_db =-40 
    ...
    infer_tool_copy_test.mkdir(["raw", resultfolder])
    
    infer_tool_copy_test.fill_a_to_b(keys, clean_names)
    for clean_name, tran in zip(clean_names, keys):
        raw_audio_path = f"{clean_name}"
        if "." not in raw_audio_path:
            raw_audio_path += ".wav"
        infer_tool_copy_test.format_wav(raw_audio_path)
        # for spk in spk_list:
        kwarg = {
            "raw_audio_path" : raw_audio_path,
            "target_singer_wav" : target_singer_wav,
            "tran" : tran,
        ...
        if args.teacher:
            ...
            isdiffusion= "como"
        res_path = f'{resultfolder}/{clean_name.split("/")[-1]}_{target_singer_wav.split("/")[-1]}_{isdiffusion}_{step_num}.{wav_format}'
