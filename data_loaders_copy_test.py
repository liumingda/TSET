...
class AudioDataset(Dataset):
    def __init__(
        ...
        # self.SpkExtractor = SpkExtractor
        ...
        with open(filelists,"r", encoding='utf8') as f:
        ...
        # self.model = SpkExtractor(input_dim=80, embedding_dim=256, sample_rate=16000, n_mels=80, target_time_steps=375)
        # self.model.load_state_dict(torch.load('D:\\liumingda\\SVC\\code\\comosvc_ori\\CoMoSVC\\spk_embedding_model\\spk_extractor.pth'))  # 加载保存的模型
        # self.model.eval()  # 设置为评估模式

        # 确保模型在CPU上执行，避免数据传输错误
        ...
        for name_ext in tqdm(self.paths, total=len(self.paths)):
            ...
            # waveform_audio_path = audio_norm.unsqueeze(0)
            # print(waveform_audio_path.shape)
            # tt
    ...
    def get_data(self, name_ext, data_buffer):
        ...
        # audio_path = self.filelists
        audio_path = data_buffer.get('audio_path')
        # tt
        
        return dict(mel=mel, f0=f0_frames, volume=volume_frames, units=units, audio_path=audio_path, aug_shift=aug_shift, name=name, name_ext=name_ext)
