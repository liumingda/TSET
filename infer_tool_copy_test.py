...
import utils
from ComoSVC_copy_test import load_model_vocoder
import slicer
...
class Svc(object):
    ...
    def infer(self, tran, raw_path, target_singer_wav):
        ...
        target_singer, _ = torchaudio.load(target_singer_wav)
        ...
        speaker_id = speech2spk_embed(target_singer[0])
        # print(speaker_id.shape)
      
        # sid = torch.LongTensor([speaker_id]).to(self.dev).unsqueeze(0)
        ...
        with torch.no_grad():
            ...
            c = c.transpose(-1,-2)
            audio_mel = self.diffusion_model(c, f0, vol,audio_path = speaker_id,gt_spec=audio_mel,infer=True)
    ...
    def slice_inference(self,
        ...
        for (slice_tag, data) in audio_data:
            ...
            for k,dat in enumerate(datas):
                ...
                raw_path.seek(0)
                out_audio, out_sr, out_frame = self.infer(tran, raw_path, target_singer_wav)
